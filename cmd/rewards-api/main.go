...
func main() {
	...
	case "calculate":
		...
		baselineRewardClient := services.NewBaselineRewardService(&settings, transferService, services.NewDeviceDataClient(&settings), deviceClient, definitionsClient, week, &logger, settings.Environment != "prod")
