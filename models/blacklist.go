...
func (o *Blacklist) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(blacklistPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert blacklist, could not build conflict column list")
			}

			conflict = make([]string, len(blacklistPrimaryKeyColumns))
			copy(conflict, blacklistPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"rewards_api\".\"blacklist\"", updateOnConflict, ret, update, conflict, insert, opts...)
