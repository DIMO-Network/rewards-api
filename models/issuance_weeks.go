...
func (o *IssuanceWeek) GetRewards() RewardSlice {
	if o == nil {
		return nil
	}
...
func (issuanceWeekL) LoadReferrals(ctx context.Context, e boil.ContextExecutor, singular bool, maybeIssuanceWeek interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`rewards_api.referrals.issuance_week_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (issuanceWeekL) LoadRewards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeIssuanceWeek interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`rewards_api.rewards.issuance_week_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *IssuanceWeek) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(issuanceWeekAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(issuanceWeekPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert issuance_weeks, could not build conflict column list")
			}

			conflict = make([]string, len(issuanceWeekPrimaryKeyColumns))
			copy(conflict, issuanceWeekPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"rewards_api\".\"issuance_weeks\"", updateOnConflict, ret, update, conflict, insert, opts...)
