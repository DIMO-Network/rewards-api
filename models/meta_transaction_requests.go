...
var MetaTransactionRequestRels = struct {
	TransactionAttestations               string
	RequestReferrals                      string
	TransferMetaTransactionRequestRewards string
}{
	TransactionAttestations:               "TransactionAttestations",
...
type metaTransactionRequestR struct {
	TransactionAttestations               AttestationSlice `boil:"TransactionAttestations" json:"TransactionAttestations" toml:"TransactionAttestations" yaml:"TransactionAttestations"`
...
func (r *metaTransactionRequestR) GetTransactionAttestations() AttestationSlice {
	if r == nil {
		return nil
	}
	return r.TransactionAttestations
}
...
// TransactionAttestations retrieves all the attestation's Attestations with an executor via transaction_id column.
func (o *MetaTransactionRequest) TransactionAttestations(mods ...qm.QueryMod) attestationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"rewards_api\".\"attestations\".\"transaction_id\"=?", o.ID),
	)

	return Attestations(queryMods...)
}
...
// LoadTransactionAttestations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (metaTransactionRequestL) LoadTransactionAttestations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMetaTransactionRequest interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`rewards_api.attestations`),
		qm.WhereIn(`rewards_api.attestations.transaction_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load attestations")
	}

	var resultSlice []*Attestation
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice attestations")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on attestations")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for attestations")
	}
	...
	if singular {
		object.R.TransactionAttestations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &attestationR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TransactionID {
				local.R.TransactionAttestations = append(local.R.TransactionAttestations, foreign)
				if foreign.R == nil {
					foreign.R = &attestationR{}
				}
...
// AddTransactionAttestations adds the given related objects to the existing relationships
// of the meta_transaction_request, optionally inserting them as new records.
// Appends related to o.R.TransactionAttestations.
// Sets related.R.Transaction appropriately.
func (o *MetaTransactionRequest) AddTransactionAttestations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Attestation) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.TransactionID, rel.Root}
	...
	if o.R == nil {
		o.R = &metaTransactionRequestR{
			TransactionAttestations: related,
		}
	} else {
		o.R.TransactionAttestations = append(o.R.TransactionAttestations, related...)
	}
