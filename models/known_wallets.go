...
func (o *KnownWallet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(knownWalletAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(knownWalletPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert known_wallets, could not build conflict column list")
			}

			conflict = make([]string, len(knownWalletPrimaryKeyColumns))
			copy(conflict, knownWalletPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"rewards_api\".\"known_wallets\"", updateOnConflict, ret, update, conflict, insert, opts...)
