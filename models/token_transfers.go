...
var (
	tokenTransferAllColumns            = []string{"chain_id", "address_from", "address_to", "amount", "transaction_hash", "log_index", "block_timestamp", "updated_at"}
	tokenTransferColumnsWithoutDefault = []string{"chain_id", "address_from", "address_to", "amount", "transaction_hash", "log_index", "block_timestamp"}
	tokenTransferColumnsWithDefault    = []string{"updated_at"}
	tokenTransferPrimaryKeyColumns     = []string{"chain_id", "transaction_hash", "log_index"}
...
func FindTokenTransfer(ctx context.Context, exec boil.ContextExecutor, chainID int64, transactionHash []byte, logIndex int, selectCols ...string) (*TokenTransfer, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"rewards_api\".\"token_transfers\" where \"chain_id\"=$1 AND \"transaction_hash\"=$2 AND \"log_index\"=$3", sel,
	)
...
func (o *TokenTransfer) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(tokenTransferAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(tokenTransferPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert token_transfers, could not build conflict column list")
			}

			conflict = make([]string, len(tokenTransferPrimaryKeyColumns))
			copy(conflict, tokenTransferPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"rewards_api\".\"token_transfers\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *TokenTransfer) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"rewards_api\".\"token_transfers\" WHERE \"chain_id\"=$1 AND \"transaction_hash\"=$2 AND \"log_index\"=$3"
...
func (o *TokenTransfer) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindTokenTransfer(ctx, exec, o.ChainID, o.TransactionHash, o.LogIndex)
	if err != nil {
		return err
	}
...
func TokenTransferExists(ctx context.Context, exec boil.ContextExecutor, chainID int64, transactionHash []byte, logIndex int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"rewards_api\".\"token_transfers\" where \"chain_id\"=$1 AND \"transaction_hash\"=$2 AND \"log_index\"=$3 limit 1)"
...
func (o *TokenTransfer) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return TokenTransferExists(ctx, exec, o.ChainID, o.TransactionHash, o.LogIndex)
}
