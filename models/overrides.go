...
func (o *Override) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(overrideAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(overridePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert overrides, could not build conflict column list")
			}

			conflict = make([]string, len(overridePrimaryKeyColumns))
			copy(conflict, overridePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"rewards_api\".\"overrides\"", updateOnConflict, ret, update, conflict, insert, opts...)
