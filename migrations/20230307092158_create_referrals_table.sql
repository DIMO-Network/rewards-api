...
CREATE TABLE referrals(
    referee bytea NOT NULL
        CONSTRAINT referrals_referee_check CHECK (length(referee) = 20),
    referrer bytea NOT NULL
        CONSTRAINT referrals_referrer_check CHECK (length(referrer) = 20),
    
    transfer_successful BOOLEAN,
    transfer_failure_reason referrals_transfer_failure_reason,

    request_id char(27) NOT NULL
        CONSTRAINT referrals_request_id_fkey REFERENCES meta_transaction_requests(id),
